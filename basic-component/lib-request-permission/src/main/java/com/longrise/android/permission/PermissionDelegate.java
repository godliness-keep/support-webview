...
public final class PermissionDelegate extends Fragment {
    ...
    private static final TreeMap<Integer, IRequestPermissionListener> PERMISSION_LISTENER = new TreeMap<>();
    ...
    public void request(String[] permissions, IRequestPermissionListener permissionListener) {
        if (requestPermission(permissions)) {
            permissionListener.onResult(permissions, createGrantResults(permissions.length), (Activity) mHost);
        } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        final IRequestPermissionListener permissionListener = PERMISSION_LISTENER.remove(requestCode);
        if (permissionListener != null) {
            permissionListener.onResult(permissions, grantResults, (Activity) mHost);
        }
    }
